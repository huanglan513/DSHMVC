...
namespace DSHOrder.Web.Controllers
{
    public class OrderDetailController : BaseController
    {
        ...
        public ActionResult Index(int id, int pageIndex=1, string BuyerName=null, string Addr=null, DateTime? OnlineDate=null, DateTime? DealDateFrom=null, DateTime? DealDateTo=null, int dealType = 0)
        {
            ...
            if (gbi != null)
            {
                ...
                IList<OrderDetail> orderDetailList = odService.GetByContidion(paging, id, BuyerName, Addr, OnlineDate, DealDateFrom, DealDateTo, dealType, null);
        ...
        public ActionResult ReadonlyRefund(int id)
        {
            OrderDetail orderDetail = GetOrderValues(id);
            return View("ReadonlyRefund", orderDetail);
        }
        ...
        public ActionResult Refund(OrderDetail model,FormCollection collection)
        {
            if (ModelState.IsValid)
            {
                ...
                orderDetail.IssueType = Convert.ToInt32(collection["IssueType"]);//model.IssueType;
                ...
                if (Convert.ToBoolean(collection["CheckOfflinePay"].Split(',')[0]))
                {
        ...
        [HttpPost]
        [ActionName("Refund")]
        [MultiButton(Name = "btnUpdateFTaobao")]
        public ActionResult UpdateFTaobao(OrderDetail model, FormCollection collection)
        {
            OrderDetail orderDetail = GetOrderValues(model.OrderDetailID);
            Taobao.OneOrderSync oos = new Taobao.OneOrderSync(User.Identity.Name, Int64.Parse(orderDetail.Tid), orderDetail.OrderDetailID);
            oos.InnerExecute();

            orderDetail = GetOrderValues(model.OrderDetailID);
            return View(orderDetail);
        }
        ...
        private OrderDetail GetOrderValues(int id)
        {
            ...
            ViewData["GroupByGroupStatus"] = groupByGroup.Status;
            ...
            ViewData["IssueTypeList"] = new SelectList(issueTypeList, "CodeValue", "CodeDesc", orderDetail.IssueType);
